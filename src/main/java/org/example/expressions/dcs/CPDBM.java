...
public final class CPDBM implements Comparable<CPDBM> {
    ...
    public CPDBM(ConstraintSet constraintSet, PDBM pdbm) {
        ...
        logger.debug("创建 CPDBM: (C: {}, D: {})", constraintSet, pdbm);
    }
    ...
    public Set<CPDBM> addGuard(AtomicGuard newGuard, Z3Oracle oracle) {
        logger.debug("CPDBM.addGuard: 尝试添加守卫 {} 到 {}", newGuard, this);
        // 调用 PDBM 的 addGuard 方法，它会返回 (ConstraintSet, PDBM) 对的列表
        List<Map.Entry<ConstraintSet, PDBM>> pdbmResults = this.pdbm.addGuard(newGuard, this.constraintSet, oracle);

        // 将 PDBM 返回的对转换为 CPDBM 集合
        Set<CPDBM> resultCPDBMs = pdbmResults.stream()
                .map(entry -> new CPDBM(entry.getKey(), entry.getValue()))
                .filter(cpdbm -> !cpdbm.isEmpty(oracle)) // 过滤掉语义为空的 CPDBM
                .collect(Collectors.toSet());

        logger.debug("CPDBM.addGuard: 返回 {} 个 CPDBM。", resultCPDBMs.size());
        return resultCPDBMs;
    }
    ...
    public Set<CPDBM> canonical(Z3Oracle oracle) {
        logger.debug("CPDBM.canonical: 规范化 {}", this);
        // 调用 PDBM 内部的 canonical 方法，它会返回 (ConstraintSet, PDBM) 对的列表
        List<Map.Entry<ConstraintSet, PDBM>> pdbmResults = this.pdbm.canonical(this.constraintSet, oracle);
        ...
        logger.debug("CPDBM.canonical: 返回 {} 个规范化 CPDBM。", canonicalCPDBMs.size());
    ...
    public CPDBM reset(ResetSet resetSet) {
        logger.debug("CPDBM.reset: 应用重置 {} 到 {}", resetSet, this);
    ...
    public boolean isEmpty(Z3Oracle oracle) {
        logger.debug("CPDBM.isEmpty: 检查 CPDBM {} 是否为空。", this);
        ...
        if (!C_satisfiable) {
            logger.debug("CPDBM.isEmpty: ConstraintSet {} 不可满足，CPDBM 为空。", this.constraintSet);
        ...
        logger.debug("CPDBM.isEmpty: PDBM 在 ConstraintSet {} 下是否为空：{}", this.constraintSet, pdbmIsEmpty);
        return pdbmIsEmpty;
    }
